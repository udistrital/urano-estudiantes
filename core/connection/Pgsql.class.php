 ...
 * @version 1.1
 * @author juusechec
 * @description se agrega caché con memcached
 *      
 */
...
class Pgsql extends ConectorDb {
    ...
    function registro_db($cadena, $numeroRegistros = 0) {
         ...
         */
        /**
         * Se agrega un segmento de código que usa siempre Memcached cuando encuentra en el
         * primer carácter del SQL las etiquetas qc y qc_ttl esto al estilo de mysqlnd-qc
         * http://php.net/manual/es/mysqlnd-qc.constants.php
         * Usa un hash (cualquiera) para identificar a la sentencia SQL (como key).
         */
        // /*qc=on*//*qc_ttl=5*/SELECT * FROM table
        
        if (preg_match("/\/\*qc=(.*?)\*\//", $cadena, $coincidencias) && class_exists('Memcached')) {//"Se encontro una coincidencia."
        	//array( 0=>"/*qc=on*/", 1=>"on" )
        	$qc = $coincidencias[1];
        	if ($qc == 'on'){
        		$mc = new Memcached();
        		$mc->addServer('localhost', 11211);
        		$key = hash('ripemd160', $cadena);
        		$rec = $mc->get($key);
        		if ($rec) {//'recuperado de memcached'
        			$this->registro = $rec;
        			$this->conteo = count($this->registro);
        			return $this->conteo;
        			//$mc->delete("foo");
        		} else {//'guardando en memcached';
        			@$busqueda = pg_query ( $this->enlace, $cadena );
        			if ($busqueda) {
        				 $array = $this->procesarResultado ( $busqueda, $numeroRegistros );
        				 //array( 0=>"/*qc_ttl=5*/", 1=>"5" )
        				 $qc_ttl = (preg_match("/\/\*qc_ttl=(.*?)\*\//", $cadena, $coincidencias))?$coincidencias[1]:'30';//30 secs default
						 $aGuardar = (isset($this->registro))?$this->registro:array();
        				 $mc->set($key, $aGuardar, $qc_ttl);
        				 return $array;
        			} else {
        				unset ( $this->registro );
        ...
        } else { //"No se encontro ninguna coincidencia."
        	@$busqueda = pg_query ( $this->enlace, $cadena );
	        if ($busqueda) {
